...
- перед каждым API-запросом бот проверяет TTL авторизации и наличие cookies;
- если сессия `3x-ui` протухла или API вернул `401/403/404`, бот один раз выполняет re-login и повторяет запрос;
